...
void KALMAN<Nstate, Nobs, Ncom, MemF>::_update(BLA::Matrix<Nobs> obs, BLA::Matrix<Nstate> comstate)
{
  if (KALMAN_CHECK)
  {
    for (int i = 0; i < Nobs; i++)
    {
      if (isnan(obs(i)) || isinf(obs(i)))
      {
        if (KALMAN_VERBOSE)
        {
          Serial.println("KALMAN:ERROR: observation has nan or inf values");
        }
        status = 1;
        return;
      }
    ...
    if (KALMAN_CHECK)
    {
      for (int i = 0; i < Nstate; i++)
      {
        if (isnan(this->x(i)) || isinf(this->x(i)))
        {
          if (KALMAN_VERBOSE)
          {
            Serial.println("KALMAN:ERROR: estimated vector has nan or inf values");
          }
          status = 1;
          return;
        }
  ...
  else
  {
    if (KALMAN_VERBOSE)
    {
      Serial.println("KALMAN:ERROR: could not invert S matrix. Try to reset P matrix.");
    }
  ...
  if (KALMAN_CHECK)
  {
    for (int i = 0; i < Ncom; i++)
    {
      if (isnan(com(i)) || isinf(com(i)))
      {
        if (KALMAN_VERBOSE)
        {
          Serial.println("KALMAN:ERROR: command has nan or inf values");
        }
        status = 1;
        return;
      }
